...
/******************************************************************************/
/**
    Initialize the key schedule of an AES (Rijndael) block cipher.
    The initilaized key can be used only for encrypt or decrypt.
    If the same key bytes need to be used for both, two psAesKey_t structures
    should be initialized.

    @param[in,out] key Pointer to an allocated key structure to be initialized.
    @param[in] ckey The symmetric key as bytes.
    @param[in] keylen The number of bytes in 'ckey'.
    @param[in] flags PS_AES_ENCRYPT or PS_AES_DECRYPT.
    @return PS_SUCCESS on success, < 0 on failure.

    @security psAesClearBlockKey() should be called as soon as the key is no
    longer needed.
    @security If 'ckey' is no longer needed after this api call, it should be
    cleared.
 */
...
# ifdef CRYPTO_ASSERT
    ...
    if (keylen != AES128_KEYLEN && keylen != AES192_KEYLEN &&
...
# endif
    /*
        Setup the forward (encrypt) key schedule. If flags & PS_AES_DECRYPT we
        will calculate the inverse (decrypt) key schedule from this forward
    ...
    case 24:
        key->rounds = 12;
        LOAD32H(rk[4], ckey + 16);
        LOAD32H(rk[5], ckey + 20);
        for (;; )
        {
# ifdef _MSC_VER
            temp = key->skey[rk - key->skey + 5];
# else
            temp = rk[5];
# endif        /* _MSC_VER */
    ...
    case 32:
        ...
        LOAD32H(rk[4], ckey + 16);
        LOAD32H(rk[5], ckey + 20);
        LOAD32H(rk[6], ckey + 24);
        LOAD32H(rk[7], ckey + 28);
        for (;; )
        {
# ifdef _MSC_VER
            temp = key->skey[rk - key->skey + 7];
# else
            temp = rk[7];
# endif        /* _MSC_VER */
...
static void _aes_ecb_encrypt(psAesKey_t *key, const unsigned char *pt,
                             unsigned char *ct);
void psAesEncryptBlock(psAesKey_t *key, const unsigned char *pt,
    unsigned char *ct)
{
    _aes_ecb_encrypt(key, pt, ct);
...
# ifdef CRYPTO_ASSERT
    if (pt == NULL || ct == NULL || key == NULL || key->type != PS_AES_ENCRYPT)
    {
        return;
    }
# endif
...
# ifdef USE_BURN_STACK
static void _aes_ecb_decrypt(psAesKey_t *key, const unsigned char *ct,
                             unsigned char *pt);
void psAesDecryptBlock(psAesKey_t *key, const unsigned char *ct,
    unsigned char *pt)
{
    _aes_ecb_decrypt(key, ct, pt);
    psBurnStack(sizeof(uint32) * 8 + sizeof(uint32 *) + sizeof(int32) * 2);
}
static void _aes_ecb_decrypt(psAesKey_t *key, const unsigned char *ct,
    unsigned char *pt)
...
# ifdef CRYPTO_ASSERT
    if (pt == NULL || ct == NULL || key == NULL || key->type != PS_AES_DECRYPT)
    {
        return;
    }
# endif
...
/******************************************************************************/
/**
    Convert a forward (encrypt) key schedule into an inverse (decrypt)
    key schedule.
    @note Inline since it is only called from one internal function, and
...
# endif /* PS_AES_IMPROVE_PERF_INCREASE_CODESIZE */
    ...
    *rk   = *rrk;
    ...
    memset_s(ek, sizeof(ek), 0x0, sizeof(ek));
}

#endif /* USE_MATRIX_AES_BLOCK */
