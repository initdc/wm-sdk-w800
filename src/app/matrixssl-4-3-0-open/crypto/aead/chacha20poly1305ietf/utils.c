...
psSodium_memzero_as_a_weak_symbol_to_prevent_lto(void *const  pnt,
                                                  const size_t len)
{
...
void
psSodium_memzero(void *const pnt, const size_t len)
{
# ifdef _WIN32
...
# elif HAVE_WEAK_SYMBOLS
    psSodium_memzero_as_a_weak_symbol_to_prevent_lto(pnt, len);
# else
...
# ifdef HAVE_WEAK_SYMBOLS
__attribute__((weak)) void
psSodium_dummy_symbol_to_prevent_memcmp_lto(const unsigned char *b1,
...
int
psSodium_memcmp(const void *const b1_, const void *const b2_, size_t len)
{
# ifdef HAVE_WEAK_SYMBOLS
...
# if HAVE_WEAK_SYMBOLS
    psSodium_dummy_symbol_to_prevent_memcmp_lto(b1, b2, len);
# endif
    for (i = 0U; i < len; i++) {
        d |= b1[i] ^ b2[i];
    }
...
# ifdef HAVE_WEAK_SYMBOLS
__attribute__((weak)) void
psSodium_dummy_symbol_to_prevent_compare_lto(const unsigned char *b1,
                                              const unsigned char *b2,
                                              const size_t         len)
{
    (void) b1;
...
int
psSodium_compare(const unsigned char *b1_, const unsigned char *b2_, size_t len)
{
# ifdef HAVE_WEAK_SYMBOLS
...
# if HAVE_WEAK_SYMBOLS
    psSodium_dummy_symbol_to_prevent_compare_lto(b1, b2, len);
# endif
    i = len;
    while (i != 0U) {
        i--;
...
int
psSodium_is_zero(const unsigned char *n, const size_t nlen)
{
    size_t        i;
    unsigned char d = 0U;
...
void
psSodium_increment(unsigned char *n, const size_t nlen)
{
    size_t        i = 0U;
    uint_fast16_t c = 1U;
...
void
psSodium_add(unsigned char *a, const unsigned char *b, const size_t len)
{
    size_t        i = 0U;
    uint_fast16_t c = 0U;
...
char *
psSodium_bin2hex(char *const hex, const size_t hex_maxlen,
...
int
psSodium_hex2bin(unsigned char *const bin, const size_t bin_maxlen,
...
int
psSodium_mlock(void *const addr, const size_t len)
{
...
int
psSodium_munlock(void *const addr, const size_t len)
{
    psSodium_memzero(addr, len);
...
static int
psMprotect_noaccess(void *ptr, size_t size)
{
# ifdef HAVE_MPROTECT
...
static int
psMprotect_readonly(void *ptr, size_t size)
{
# ifdef HAVE_MPROTECT
...
static int
psMprotect_readwrite(void *ptr, size_t size)
{
# ifdef HAVE_MPROTECT
...
__attribute__((malloc)) void *
psSodium_allocarray(size_t count, size_t size)
{
    size_t total_size;
    ...
    return psSodium_malloc(total_size);
}
...
#  ifndef HAVE_PAGE_PROTECTION
    if (psSodium_memcmp(unprotected_ptr + unprotected_size, canary, sizeof canary) != 0) {
        _out_of_bounds();
    }
#  endif
    psSodium_munlock(unprotected_ptr, unprotected_size);
...
int
psSodium_mprotect_noaccess(void *ptr)
{
    return psSodium_mprotect(ptr, psMprotect_noaccess);
}

int
psSodium_mprotect_readonly(void *ptr)
{
    return psSodium_mprotect(ptr, psMprotect_readonly);
}

int
psSodium_mprotect_readwrite(void *ptr)
{
